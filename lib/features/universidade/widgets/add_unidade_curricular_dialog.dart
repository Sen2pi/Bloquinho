...
class AddUnidadeCurricularDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AddUnidadeCurricularDialog> createState() =>
      _AddUnidadeCurricularDialogState();
}

class _AddUnidadeCurricularDialogState
    extends ConsumerState<AddUnidadeCurricularDialog> {
  ...
  int? _semestre;
  int? _creditos;
  String _statusConclusao = 'Em Curso';
  ...
  void initState() {
    ...
    if (widget.unidade != null) {
      ...
      _statusConclusao = widget.unidade!.statusConclusao;
      _notaFinalController.text = widget.unidade!.notaFinal?.toString() ?? '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text(widget.unidade == null
                ...
                cursosAsync.when(
                  data: (cursos) => DropdownButtonFormField<String>(
                    ...
                    items: cursos
                        ...
                        .toList(),
                    onChanged: (value) =>
                        setState(() => _selectedCursoId = value),
                    validator: (value) =>
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        value: _statusConclusao,
                        decoration: const InputDecoration(
                          labelText: 'Status',
                          border: OutlineInputBorder(),
                        ),
                        items: ['Em Curso', 'Concluída', 'Suspensa', 'Reprovada']
                            .map((status) => DropdownMenuItem(
                                  value: status,
                                  child: Text(status),
                                ))
                            .toList(),
                        onChanged: (value) => setState(() => _statusConclusao = value ?? 'Em Curso'),
                      ),
                    ),
                    const SizedBox(width: 16),
                    Expanded(
  ...
  void _saveUnidade() {
    if (_formKey.currentState!.validate() && _selectedCursoId != null) {
      final unidade = widget.unidade?.copyWith(
            ...
            codigo:
                _codigoController.text.isEmpty ? null : _codigoController.text,
            ...
            semestre: _semestre,
            creditos: _creditos,
            statusConclusao: _statusConclusao,

...
class PasswordCsvService {
  ...
  static Future<List<PasswordEntry>> importFromCsv() async {
    try {
      ...
      final urlIndex = _findColumnIndex(
          header, ['url', 'website', 'site', 'domain', 'web site', 'link']);
      final usernameIndex = _findColumnIndex(header,
          ['username', 'login', 'user', 'email', 'login name', 'account']);
      final passwordIndex = _findColumnIndex(
          header, ['password', 'pass', 'senha', 'secret', 'passphrase']);
      ...
      final folderIndex = _findColumnIndex(
          header, ['folder', 'category', 'categoria', 'group', 'grouping']);
      final typeIndex = _findColumnIndex(
          header, ['type', 'tipo', 'item type', 'record type']);
      ...
      for (int i = 1; i < csvData.length; i++) {
        ...
        try {
          ...
          final entry = PasswordEntry(
            ...
            category: categoryValue.isEmpty
                ? 'Social'
                : categoryValue, // Categoria padrão "Social"
            tags: [],
            createdAt: now,
            updatedAt: now,
            isFavorite: false,
      ...
      if (passwords.isEmpty && totalRows > 0) {
        ...
        if (nameIndex == -1) {
          suggestions.add(
              'Coluna de nome não encontrada. Colunas esperadas: name, title, item name, nome, site name, service');
        }
        if (passwordIndex == -1) {
          suggestions.add(
              'Coluna de senha não encontrada. Colunas esperadas: password, pass, senha, secret, passphrase');
        }
        if (usernameIndex == -1) {
          suggestions.add(
              'Coluna de usuário não encontrada. Colunas esperadas: username, login, user, email, login name, account');
        }

        if (suggestions.isNotEmpty) {
          final errorMsg =
              'Formato do CSV não reconhecido:\n${suggestions.join('\n')}\n\nHeader encontrado: $header';
  ...
  static String _detectCsvSeparator(String content) {
    ...
    final mostCommon =
        separatorCounts.entries.reduce((a, b) => a.value > b.value ? a : b);
  ...
  static Future<bool> validateCsvFormat(String filePath) async {
    try {
      ...
      final hasUsername =
          header.any((col) => ['username', 'login'].contains(col));
